...
/* ── Lifecycle diagram click-to-reveal (used in csfb.html) ── */
const CSFB_SEASONS = ['march', 'june', 'sep', 'dec'];
const OSR_SEASONS  = ['march', 'june', 'sep', 'dec'];

function showSeason(diagram, season) {
  const seasons = diagram === 'csfb' ? CSFB_SEASONS : OSR_SEASONS;

  seasons.forEach(s => {
    const panel = document.getElementById(diagram + '-' + s);
    if (panel) panel.classList.toggle('visible', s === season);

    const hotspot = document.getElementById(diagram + '-hotspot-' + s);
    if (hotspot) hotspot.classList.toggle('active', s === season);
  });
}
...
/* ── Methodology protein label toggle ── */
function toggleProtein(id) {
  const ids = ['prot-a', 'prot-b'];
  const wasVisible = document.getElementById(id)?.style.display === 'block';
  ids.forEach(pid => {
